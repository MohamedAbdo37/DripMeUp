...
//    private Address address;
    ...
    public void updatePassword(String password){
        if(this.getRole() == Role.ADMIN) {
            AdminEntity admin = adminRepository.findByUserName(this.getUsername()).get(0);
            admin.setPassword(password);
            databaseService.saveOrUpdate(admin);
        }
        if(this.getRole() == Role.USER) {
            UserEntity user = userRepository.findByEmail(this.getEmail());
    ...
    public EntityIF getTuble(){
        if (this.getRole() == Role.ADMIN)
            return adminRepository.findByUserName(this.getUsername()).get(0);
        if (this.getRole() == Role.USER)
